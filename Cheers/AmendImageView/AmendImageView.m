...
@implementation UIImageView(AmendImageView)
...
- (void)setAmendURL:(NSURL *)amendURL{
    [[TactImageLoader sharedLoader] loadImageWithURL:amendURL target:self action:@selector(setImage:)];
}
...
-(void) setAmendURL:(NSURL *)amendURL
{
    UIImage *image = [[TactImageLoader sharedLoader].cache objectForKey:amendURL];
    if (image)
    {
        self.image = image;
        return;
    }
    super.amendURL = amendURL;
    if ( self.showActivityIndicator && !self.image && amendURL)
    {
        ...
        if (self.activityView == nil)
        {
            ...
            NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:amendURL
                                                            ...
                                                            [[NSOperationQueue mainQueue] addOperationWithBlock:^
                                                             {
                                                                 ...
                                                                 if(httpResponse.statusCode == 200){
                                                                     ...
                                                                     self.image = [UIImage imageNamed:@"errorplacehoader.png"];
                                                                 }
...
- (void)dealloc
{
    ...
    [[TactImageLoader sharedLoader] cancelLoadingURL:self.amendURL target:self];
}
